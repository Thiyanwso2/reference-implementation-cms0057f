...
isolated Condition[] conditions = [];
isolated int createOperationNextId = 12344;

public isolated function create(json payload) returns r4:FHIRError|Condition {
    Condition|error condition = parser:parse(payload, Condition).ensureType();
...
public isolated function getById(string id) returns r4:FHIRError|Condition {
    lock {
...
public isolated function search(map<string[]>? searchParameters = ()) returns r4:FHIRError|r4:Bundle {
    ...
    if searchParameters is map<string[]> {
        foreach var 'key in searchParameters.keys() {
            match 'key {
                "_id" => {
                    Condition byId = check getById(searchParameters.get('key)[0]);
                    bundle.entry = [
                        {
                            'resource: byId
                        }
...
//         };
//         Condition condition = check parser:parse(conditionJson, Condition).ensureType();
//         conditions.push(condition.clone());
//     }
// }
